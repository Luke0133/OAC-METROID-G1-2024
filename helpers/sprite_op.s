...
RESET_SPRITE_BALL:
    sh zero, 0(t0) # Reset sprite number

CHECK_MOVE_Y:
    la t1, 2(t0) # Check if samus is facing up
    bnez t1, SPRITE_UP_GROUND # t1 != 0 ? SPRITE_UP_GROUND
    la t1, 3(t0) # Check if samus is falling
    bnez t1, SPRITE_FALLING # t1 != 0 ? SPRITE_FALLING
    j CHECK_MOVE_X # Check for movement in horizontal direction
...
RESET_SPRITE_UP:
    sh zero, 0(t0) # Reset sprite number
    j CHECK_MOVE_X # movex or end update status?
